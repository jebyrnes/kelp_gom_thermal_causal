...
Supplementary Table S7. F Table results from summer mean temperature trend linear regression model.

```{r}
summary(summer_ts_mod) %>% tidy %>% coef_t_tab
```

Supplementary Table S8. Coefficient Table results from summer mean temperature trend linear regression model.


```{r}
summer_ts_max_mod <- readRDS("model_output/summer_max_temp_timeseries.rds")

Anova(summer_ts_max_mod) %>% tidy %>% f_anova_tab
```

Supplementary Table S9. F Table results from summer maximum temperature trend linear regression model.

```{r}
summary(summer_ts_max_mod) %>% tidy %>% coef_t_tab
```

Supplementary Table S10. Coefficient Table results from summer maximum temperature trend linear regression model.


```{r}
summer_dhd15_mod <- readRDS("model_output/summer_dhd_15_timeseries.rds")

Anova(summer_dhd15_mod) %>% tidy %>% f_anova_tab
```

Supplementary Table S11. F Table results from summer degree heating day above 15C regression model.

```{r}
summary(summer_dhd15_mod) %>% tidy %>% coef_t_tab
```

Supplementary Table S12. Coefficient Table results from summer degree heating day above 15C trend linear regression model.


```{r}
summer_dhd20_mod <- readRDS("model_output/summer_dhd_20_timeseries.rds")

Anova(summer_dhd20_mod) %>% tidy %>% f_anova_tab
```

Supplementary Table S13. F Table results from summer degree heating day above 20C linear regression model.

```{r}
summary(summer_dhd20_mod) %>% tidy %>% coef_t_tab
```

Supplementary Table S14. Coefficient Table results from summer degree heating day above 20C trend linear regression model.
...
Supplementary Table S15. $\chi^2$ likelihood ratio tests for trend analysis of kelp species composition over time.
...
Supplementary Table S16. $\chi^2$ likelihood ratio tests for changes in individual kelp species  over time with p-values corrected for false discovery rate.
...
Supplementary Table S17. Posthoc comparisons of differences in kelp species within each subregion between 2004 and 2018. Differences derived via simulation, and as such we include the 95% CI for each.
...
Supplementary Table S18. $\chi^2$ likelihood ratio tests for trend analysis of understory species composition over time.
...
```
Supplementary Table S19. $\chi^2$ likelihood ratio tests for changes in individual understory species  over time with p-values corrected for false discovery rate.
...
Supplementary Table S20. Posthoc comparisons of differences in understory species within each subregion between 2004 and 2018. Differences derived via simulation, and as such we include the 95% CI for each.
...
read_csv("tables/urchin_size_trends.csv") |>
    qflextable()
```
Supplementary Table S21. Trends in urchin size by region over time.   
...
read_csv("tables/urchin_size_trends_contrast.csv") |>
    qflextable()
```
Supplementary Table S22. Differences in trends in urchin sizes over time between regions. Only midcoast differs from any other regions. 